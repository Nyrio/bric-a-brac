...
# The same as cubic_spline_2d but time intervals are calculated according
# to the distance between consecutive points.
...
def main():
    """Asks the user for the name of a file with 2 lines : one for x values,
       ...
       the picture which the user selects, with a representation of the curve
    """
    ...
    t = [0]
    for i in range(1, N):
        t.append(t[-1] + math.sqrt((x[i]-x[i-1])**2 + (y[i]-y[i-1])**2))
    ...
    for i in range(NB_VALUES):
        ...
        k = max(ki for ki in range(N-1) if t[ki] <= ti[-1]) # num of the part
